...
/// Generated files from zngur code generation
pub struct GeneratedFiles {
    pub rust_file: String,
    pub primary_header: String,                 // generated.h content
    pub utility_headers: Vec<(String, String)>, // vec of (filename, content) pairs like ("zngur.h", content)
    ...
    pub fn render(self) -> GeneratedFiles {
        ...
        cpp_file.trait_defs = zng
            ...
            .map(|(key, value)| {
                (
                    key.clone(),
                    rust_file.add_builder_for_dyn_trait(value, namespace),
                )
            })
            .collect();
        ...
        for ty_def in zng.types {
                        ...
                        cpp_methods.push(CppMethod {
                            ...
                            sig: CppFnSig {
                                ...
                                inputs: constructor
                                    .inputs
                                    .iter()
                                    .map(|x| x.1.into_cpp(namespace))
                                    .collect(),
                                output: ty.into_cpp(namespace),
                            },
                        });
                        cpp_methods.push(CppMethod {
                            ...
                            sig: CppFnSig {
                                ...
                                inputs: vec![ty.into_cpp(namespace).into_ref()],
            ...
            if let RustType::Tuple(fields) = &ty_def.ty {
                if !fields.is_empty() {
                    ...
                    constructors.push(CppFnSig {
                        rust_link_name,
                        inputs: fields.iter().map(|x| x.into_cpp(namespace)).collect(),
                        output: ty.into_cpp(namespace),
                    });
                }
            }
            ...
            for method_details in ty_def.methods {
                ...
                let (rusty_inputs, inputs) = real_inputs_of_method(&method, &ty, namespace);
                ...
                cpp_methods.push(CppMethod {
                    ...
                    sig: CppFnSig {
                        rust_link_name,
                        inputs,
                        output: method.output.into_cpp(namespace),
                    },
                });
            }
            cpp_file.type_defs.push(CppTypeDefinition {
                ty: ty.into_cpp(namespace),
                                ...
                                e.insert(CppTraitDefinition::Fn {
                                    sig: CppFnSig {
                                        ...
                                        inputs: inputs
                                            ...
                                            .collect(),
                                        output: output.into_cpp(namespace),
                                    },
                                });
                            }
                        }
        ...
        for func in zng.funcs {
            ...
            cpp_file.fn_defs.push(CppFnDefinition {
                name: CppPath::from_rust_path(&func.path.path, namespace),
                sig: CppFnSig {
                    rust_link_name,
                    inputs: func
                        .inputs
                        ...
                        .map(|x| x.into_cpp(namespace))
                        .collect(),
                    output: func.output.into_cpp(namespace),
                },
            });
        }
        ...
        for impl_block in zng.extern_cpp_impls {
            ...
            cpp_file.exported_impls.push(CppExportedImplDefinition {
                tr: impl_block.tr.map(|x| x.into_cpp(namespace)),
                ty: impl_block.ty.into_cpp(namespace),
                methods: impl_block
                    .methods
                    .iter()
                    .zip(&rust_link_names)
                    .map(|(method, link_name)| {
                        let (_, inputs) = real_inputs_of_method(method, &impl_block.ty, namespace);
                        (
        ...
        let (h, zngur_h, cpp) = cpp_file.render(&zng.cpp_namespace);
        GeneratedFiles {
            rust_file: rust_file.text,
            primary_header: h,
            utility_headers: vec![("zngur.h".to_string(), zngur_h)],
            cpp_file: cpp,
        }
    }
}

fn real_inputs_of_method(
    method: &ZngurMethod,
    ty: &RustType,
    namespace: &str,
) -> (Vec<RustType>, Vec<CppType>) {
